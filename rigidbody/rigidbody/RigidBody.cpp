...
*       1.  Call setStatics() to set stepsize and g (gravity is always along the z axis)
...
*       1.  Call RKInit()
*       2.  Use linPos, linVel, rotPos and rotVel to determine forces on body.
*           Add forces and torques with provided functions, or add them to linForce and rotForce directly.
*       3.  Calculate constraint forces if necessary and add to linForce and rotForce
*       4.  Call eulerStep()
*       5.  Draw body at linPos, linVel, rotPos and rotVel.
*
...
*       10. Call RKStep4()
*       11. Draw body at linPos, linVel, rotPos and rotVel.
********************************************************************/
...
*
*   params :
*       float h               -   Runge-Kutta step size (secs)
*       float g               -   gravity (m/s/s)
*
*   Set up the Runge Kutta step size (in seconds), and gravity.
*   Step size is static, so the same value applies for all bodies.
*   You must call this before using rigid bodies.
********************************************************************/

void RigidBody::setStatics(float h, float g)
{
	RKh = h;
	RKh2 = h / 2.0;
	RKh4 = h / 4.0;
...
*       Vector3 & linPos                        -   position of other body origin(this body)
*
...
// New centre of mass = (thisCOM * thisMass + otherCOM * otherMass) / totalMass
    ...
    otherCOfM.transform(other.cOfM, rotPos, linPos);// this = rotPos * r + linPos  (this body)
    float totalMass = mass + other.mass;
    cOfM.mult(mass / totalMass);                    //
    cOfM.mac(otherCOfM, other.mass / totalMass);    //this = A + A * s   to derive the cOfM in this body coordidate
    mass = totalMass;

// now combine other moment of inertia with this one)(this body coordidate)
    float *m = rotPos.m;
...
*       Rd = diagRotPos * Ro + diagLinPos               note: old body coords
***********************************************************************************/
...
void RigidBody::diagonalise(void)
{
    Matrix3x3 identity;
    identity.identity();
    Vector3 cOfM(-Ix / mass, -Iy / mass, -Iz / mass);   // centre of mass in old coords  (note: old coords) why -?
    RigidBody old = *this;
    clear();                                            // clear: make all property to zero
    combine(old, identity, cOfM);                       // shift centre of mass to origin  ,not to diagonalise

    Matrix3x3 I(                                    // moment of inertia (old coords) not world coords
        Ixx, -Ixy, -Izx,
        -Ixy, Iyy, -Iyz,
        -Izx, -Iyz, Izz);

    Vector3 d;
    Matrix3x3::eigenvectors(I, diagRotPos, d);      // find diagonalising transform with eigen vectors(diagRotPos)
    diagRotPos.transpose();                         // ×ªÖÃ
    diagLinPos.mult(diagRotPos, cOfM);              // this = A * s  set up diagLinPos(not world coords)???have questions?
...
// now precalculate some useful numbers for speed ups
    oneOverDiagIxxRKh = RKh / diagIxx;
    oneOverDiagIyyRKh = RKh / diagIyy;
    oneOverDiagIzzRKh = RKh / diagIzz;
    oneOverDiagIxxRKh2 = RKh2 / diagIxx;
    oneOverDiagIyyRKh2 = RKh2 / diagIyy;
    oneOverDiagIzzRKh2 = RKh2 / diagIzz;
    oneOverMassRKh = RKh / mass;
    oneOverMassRKh2 = RKh2 / mass;
    oneOverMass = 1.0 / mass;
    oneOverDiagIxx = 1.0 / diagIxx;
    oneOverDiagIyy = 1.0 / diagIyy;
    oneOverDiagIzz = 1.0 / diagIzz;
    minusMG = -mass * g;
    diagIyyMinusIzzOverIxxRKh = (diagIyy - diagIzz) / diagIxx * RKh;
    diagIzzMinusIxxOverIyyRKh = (diagIzz - diagIxx) / diagIyy * RKh;
    diagIxxMinusIyyOverIzzRKh = (diagIxx - diagIyy) / diagIzz * RKh;
    diagIyyMinusIzzOverIxxRKh2 = (diagIyy - diagIzz) / diagIxx * RKh2;
    diagIzzMinusIxxOverIyyRKh2 = (diagIzz - diagIxx) / diagIyy * RKh2;
    diagIxxMinusIyyOverIzzRKh2 = (diagIxx - diagIyy) / diagIzz * RKh2;
    ...
    float linKD, float rotKD)
{
    ...
    rotPos.mult(oRotPos, M1);                       // rotPos = oRotPos * trans(diagRotPos)  ÒÉÎÊ£º£¿£¿£¿
...
void RigidBody::eulerStep(void)
{
    ...
    linVel.x += linForce.x * oneOverMassRKh;
    linVel.y += linForce.y * oneOverMassRKh;
    linVel.z += linForce.z * oneOverMassRKh;
    ...
    rotVel1.x = rotVel.x + rotVel.y * rotVel.z * diagIyyMinusIzzOverIxxRKh + rotForce.x * oneOverDiagIxxRKh;
    rotVel1.y = rotVel.y + rotVel.z * rotVel.x * diagIzzMinusIxxOverIyyRKh + rotForce.y * oneOverDiagIyyRKh;
    rotVel1.z = rotVel.z + rotVel.x * rotVel.y * diagIxxMinusIyyOverIzzRKh + rotForce.z * oneOverDiagIzzRKh;
    rotVel = rotVel1;
...
{
    linAcc1.x = linForce.x * oneOverMassRKh2;
    linAcc1.y = linForce.y * oneOverMassRKh2;
    linAcc1.z = linForce.z * oneOverMassRKh2;
    rotAcc1.x = rotVel.y * rotVel.z * diagIyyMinusIzzOverIxxRKh2 + rotForce.x * oneOverDiagIxxRKh2;
    rotAcc1.y = rotVel.z * rotVel.x * diagIzzMinusIxxOverIyyRKh2 + rotForce.y * oneOverDiagIyyRKh2;
    rotAcc1.z = rotVel.x * rotVel.y * diagIxxMinusIyyOverIzzRKh2 + rotForce.z * oneOverDiagIzzRKh2;
...
{
    linAcc2.x = linForce.x * oneOverMassRKh2;
    linAcc2.y = linForce.y * oneOverMassRKh2;
    linAcc2.z = linForce.z * oneOverMassRKh2;
    rotAcc2.x = rotVel.y * rotVel.z * diagIyyMinusIzzOverIxxRKh2 + rotForce.x * oneOverDiagIxxRKh2;
    rotAcc2.y = rotVel.z * rotVel.x * diagIzzMinusIxxOverIyyRKh2 + rotForce.y * oneOverDiagIyyRKh2;
    rotAcc2.z = rotVel.x * rotVel.y * diagIxxMinusIyyOverIzzRKh2 + rotForce.z * oneOverDiagIzzRKh2;
...
{
    linAcc3.x = linForce.x * oneOverMassRKh;
    linAcc3.y = linForce.y * oneOverMassRKh;
    linAcc3.z = linForce.z * oneOverMassRKh;
    rotAcc3.x = rotVel.y * rotVel.z * diagIyyMinusIzzOverIxxRKh + rotForce.x * oneOverDiagIxxRKh;
    rotAcc3.y = rotVel.z * rotVel.x * diagIzzMinusIxxOverIyyRKh + rotForce.y * oneOverDiagIyyRKh;
    rotAcc3.z = rotVel.x * rotVel.y * diagIxxMinusIyyOverIzzRKh + rotForce.z * oneOverDiagIzzRKh;
...
void RigidBody::RKStep4(void)
{
    ...
    rotPos.fromQuatL2(qRotPos, l2);                 // make new matrix from quaternion.
  	...
  	linVel.x = linVel1.x + (linAcc1.x + 2.0 * linAcc2.x + linAcc3.x + linForce.x * oneOverMassRKh2) * (1.0 / 3.0);
  	linVel.y = linVel1.y + (linAcc1.y + 2.0 * linAcc2.y + linAcc3.y + linForce.y * oneOverMassRKh2) * (1.0 / 3.0);
  	linVel.z = linVel1.z + (linAcc1.z + 2.0 * linAcc2.z + linAcc3.z + linForce.z * oneOverMassRKh2) * (1.0 / 3.0);

  	rotVel4.x = rotVel1.x + (rotAcc1.x + 2.0 * rotAcc2.x + rotAcc3.x + rotVel.y * rotVel.z * diagIyyMinusIzzOverIxxRKh2 + rotForce.x * oneOverDiagIxxRKh2) * (1.0 / 3.0);
  	rotVel4.y = rotVel1.y + (rotAcc1.y + 2.0 * rotAcc2.y + rotAcc3.y + rotVel.z * rotVel.x * diagIzzMinusIxxOverIyyRKh2 + rotForce.y * oneOverDiagIyyRKh2) * (1.0 / 3.0);
  	rotVel4.z = rotVel1.z + (rotAcc1.z + 2.0 * rotAcc2.z + rotAcc3.z + rotVel.x * rotVel.y * diagIxxMinusIyyOverIzzRKh2 + rotForce.z * oneOverDiagIzzRKh2) * (1.0 / 3.0);
    rotVel = rotVel4;
}
...
*       void
*
*   Force is given in world coords, point is diag body coords
***********************************************************************************/
